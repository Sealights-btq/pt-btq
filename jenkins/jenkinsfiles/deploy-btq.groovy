...
pipeline {
   agent {
    ...
    environment {
        IDENTIFIER = 'btq-pt.btq.sealights.co'
        tag = "pt_${params.tag}"
    }
     stages {
                        ...
                        IP = "${IDENTIFIER}"
                                stage("Updating Helm") {
                                    sh script: """
                                        ...
                                        ssh -o StrictHostKeyChecking=no -i ${SSH_KEY_FILE} ${SSH_USER}@internal-pt.btq.sealights.co 'bash /opt/sealights/install-btq.sh --tag=${env.tag} --buildname=${params.buildname} --labid=${params.labid} --branch=${params.branch} --token=${env.SL_TOKEN} --sl_branch=${params.branch}'
                                    """
                                }
                    }
                }
